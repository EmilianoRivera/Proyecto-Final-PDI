...
def filtrar_por_area(mascara_binaria, area_minima=120):
    """
...
# 1. Cargar y redimensionar la imagen
    ...
    kernel_grande = np.ones((9,9), np.uint8)
    mascara_objetos = cv2.morphologyEx(mascara_objetos, cv2.MORPH_OPEN, kernel_grande)

    # ---------------------------------------------------------
    # PASO 3: OPERACIÓN LÓGICA XOR PARA EXTRAER PIEDRAS
    # ---------------------------------------------------------
    # Al estar limpia la máscara de objetos, el XOR dará un resultado impecable
    mascara_piedras = cv2.bitwise_xor(mascara_objetos, mascara_frijoles)

    # ---------------------------------------------------------
    # PASO 3.5: EL TOQUE MAESTRO - FILTRADO GEOMÉTRICO DE ÁREA
    # ---------------------------------------------------------
    # Aplicamos el filtro para eliminar las medias lunas de las sombras residuales
    mascara_frijoles_limpia = filtrar_por_area(mascara_frijoles, area_minima=150)
    mascara_piedras_limpia = filtrar_por_area(mascara_piedras, area_minima=100)
    ...
    cv2.imshow('2. Binario Piedras (XOR sin Sombras)', mascara_piedras_limpia)
...
# Ejecutar programa
separar_y_descargar_binarios("img/Frijoles-bayos-piedras.jpeg")